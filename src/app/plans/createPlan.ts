...
export async function createPlan(raw: NewPlan): Promise<string> {
  ...
  return db.transaction(async (tx) => {
    // `greatest` couvre la course déjà passée, qui garde six mois pleins, et
    // ignore le null d'une date absente. `undefined` ne se lierait pas : il
    // disparaîtrait du SQL au lieu d'y passer un paramètre.
    const [plan] = await tx
      .insert(plans)
      .values({
        expiresAt: sql`greatest(now(), ${input.settings.raceDate ?? null}::timestamptz) + interval '6 months'`,
      })
    ...
    if (input.legOverrides.length > 0) {
      await tx.insert(legOverrides).values(
        input.legOverrides.map((o) => ({
          planId: plan.accessId,
    ...
    await insertSnapshots(tx, plan.accessId, input.productCodes);
